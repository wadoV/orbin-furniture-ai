/**
 * Orbin AI — 3D Closet Viewer (Professional Edition)
...
const TYPE_COLORS = {
  ...
  shelf:         0xD4B483,
  baseboard:     0x8B7355,
  // Drawer fronts: warm amber-gold — visually warm and distinct
  drawer_front:  0xE8A020,
  // Standard doors: cool slate-blue — clearly different from drawer fronts
...
export default function Viewer3D({ designs, selectedModuleId, selectedPieceIds, onSelectModule, onSelectPieces, onDeleteModule }) {
  ...
  // Orbit / Navigate mode — disables piece selection so user can rotate freely
  ...
  useEffect(() => {
    ...
    designs.forEach((design, dIdx) => {
      ...
      if (cfg.baseboard !== false && !isKitchenHigh) {
        const fr = makeMesh(iW3, BH3, T3, bodyColor, `Zócalo Frontal`, 'baseboard')
        ...
        if (isHorizontal) {
          ...
          for (let col = 0; col < 2; col++) {
            ...
            for (let i = 0; i < drawersPerCol; i++) {
              ...
              // Front — 2mm perimetral gap each side = 4mm total W and H
        ...
        {(hoveredInfo || selectedPieceIds.size > 0 || selectedModuleId) && (
          <div className="absolute top-4 left-4 bg-surface/90 backdrop-blur-xl border border-border/50 p-4 rounded-2xl pointer-events-none shadow-2xl animate-in fade-in slide-in-from-left-2 duration-300 z-10">
            ...
            <h4 className="text-sm font-bold text-white">
              {selectedPieceIds.size > 1 ? `${selectedPieceIds.size} ${t('pieces')}` : (hoveredInfo?.name || "Módulo Seleccionado")}
            </h4>
